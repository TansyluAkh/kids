...
import 'package:bebkeler/ui/screens/drag_and_drop/dnd_screen.dart';
...
Widget sliderbtn(label, icon, action, width, height, context, element, items) {
  ...
  if (action == 'playquiz') {
        ...
        MaterialPageRoute(
            builder: (context) => QuizScreen(
                quiz: Quiz.fromSubcategory(
                    element.subCategory, element.collectionPath, element.tatarCategory, items))),
      );
    };
    ...
    onSlide = () {
      Navigator.of(context).push(MaterialPageRoute(builder: (_) => DndTest(words: items,)));
    };
  }
  return SliderButton(
    action: onSlide,
